  ...
  return [
    ...
    },
    {
      ...
      execute: async () => {
        ...
        // Stage all existing files so nix develop can find flake.nix (nix requires tracked files in git repos)
        execFileSync('git', ['-C', ctx.projectDir, 'add', '-A'], { stdio: 'pipe' });
      },
    },
    {
      name: 'git-remote',
    ...
    },
    {
      ...
      execute: async () => {
        ...
        // Stage flake.lock generated by nix develop (avoids "dirty tree" warnings)
        execFileSync('git', ['-C', ctx.projectDir, 'add', '-A'], { stdio: 'pipe' });
      },
    },
    {
      ...
      execute: async () => {
        // Use absolute path to specify — uv tool install puts it in ~/.local/bin/
        // which is not on PATH inside nix develop shells
        const specifyBin = join(homedir(), '.local', 'bin', 'specify');
        execFileSync('nix', [
          'develop', ctx.projectDir, '--command',
          specifyBin, 'init', '.', '--ai', 'claude', '--force',
    ...
    },
    {
      ...
      execute: async () => {
        ...
        // Reuse pre-created session from route handler, or create a new one
        let session: { id: string };
        if (ctx.sessionId) {
          session = { id: ctx.sessionId };
        } else {
          session = createSession(ctx.dataDir, {
            projectId,
            type: 'interview',
          });
        }
