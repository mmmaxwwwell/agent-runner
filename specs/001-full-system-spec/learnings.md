...
- **`node -e` / `npx tsx -e` can't resolve project-local modules**: When running inline scripts from a temp directory, `ws` and other node_modules aren't found. Write the script to a temp `.mjs` file inside the project tree (e.g., `$PROJECT_ROOT/tests/.ws-test-XXXXXX.mjs`) so Node resolves modules from `node_modules/`.
- **Integration tests are flaky under resource pressure**: Running `npm test` (all 633 tests) followed immediately by a real server startup + API calls can cause "fetch failed" on individual integration tests. The tests pass in isolation. This is a known issue — the sequential test groups help but don't eliminate all resource contention.
- **Full system lifecycle verified**: Server build + start → API endpoints (health, projects) → session creation → SSH bridge socket (0600 perms) → WebSocket streaming + sync → SSH agent protocol relay (identities, sign, cancel) → Android APK build + install → all 14 Android integration tests → PWA assets. All 18 checks pass.
- **Android integration tests use their own server instance**: The `run.sh` script starts its own server on a separate port (default 3001). Must kill any prior server to avoid port conflicts.
